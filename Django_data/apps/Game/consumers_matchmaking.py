...
class MatchmakingPongConsumer(AsyncWebsocketConsumer):
    ...
    async def create_game_room(self, room_name, users):
        for user in users:
            ...
            await self.channel_layer.send(
                user, {"type": "matchmaking.success", "room_name": room_name}
            )
    ...
    def add_to_matchmaking_pool(self):
        MatchmakingUser.objects.create(
            channel_name=self.channel_name, user=self.scope["user"]
        )
    ...
    async def matchmaking_success(self, event):
        ...
        await self.send(
            text_data=json.dumps({"type": "match_found", "room_name": room_name})
        )
