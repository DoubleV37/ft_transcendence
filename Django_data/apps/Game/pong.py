    ...
    def powerup_run(self):
        ...
        if self.powerup_count == [0, 0]:
            if self.player_size[0] > 150:
                self.player_size[0] -= 0.5
            if self.player_size[1] > 150:
                self.player_size[1] -= 0.5
        else:
        ...
        if self.powerup_pos[1] > (900 - (self.powerup_size / 2)) or self.powerup_pos[
            1
        ] < (self.powerup_size / 2):
            self.powerup_speed *= -1
        elif (
            (self.powerup_pos[0] - (self.powerup_size / 2)) < self.ball_pos[0]
            and self.ball_pos[0] < (self.powerup_pos[0] + (self.powerup_size / 2))
            and (self.powerup_pos[1] - (self.powerup_size / 2)) < self.ball_pos[1]
            and self.ball_pos[1] < (self.powerup_pos[1] + (self.powerup_size / 2))
        ):
