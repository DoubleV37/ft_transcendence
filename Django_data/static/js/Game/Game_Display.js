...
function drawBallXY (x, y) {
  const ballX = x * gameCanvas.canvas.width;
...
function drawPowerup (y) {
  ...
  const powerupX = gameCanvas.canvas.width * (600 - gameCanvas.powerupsize * 900 / 2) / 1200;
  const powerupY = (y - gameCanvas.powerupsize / 2) * gameCanvas.canvas.height;
...
function drawVerticalDashedBorder () {
  const middleX = gameCanvas.canvas.width / 2;
...
function drawImpactAnimation (x, y) {
  let radius = 100; // initial radius of the circle
...
function draw (data) {
  ...
  if (gameCanvas.powerup) { drawPowerup(data.powerupY); }
  if (data.ballX < 0.01) {
